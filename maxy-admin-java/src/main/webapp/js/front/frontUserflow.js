$(function () {
    ...
    $(document).on('click', '#frontUserFlowWrap .btn_userflow_group_by, #userAnalysisPopup .btn_userflow_group_by', function (event) {
        try {
            ...
            if (!id.includes('Cancel') && !id.includes('Sum')) {
                ...
                if ($(this).hasClass('hidden')) {
                    ...
                    $target.each(function (index, item) {
                        // 팝업 열리도록
                        $(this).parent().parent('.page').on('click', frontUserflow.openPopup)
                        // flow order 보이도록
                        $(item).show()
                        // 체크박스 지움
    ...
    $(document).on('click',
        '#frontUserFlowWrap .user_behavior_analysis_wrap > .folding_btn', function () {
...
const frontUserflow = {
    openPopup(event) {
        ...
        const params = {
            ...
            from: $target.data('page-start-tm'),
            to: $target.data('page-end-tm'),
        ...
        const options = {
            ...
            appStartTm: $target.closest('.page_wrap_wrap').eq(0).find('.page').data('page-start-tm') || '' // 세션리플레이 조회시 사용
        }

        new MaxyFrontPopupPageDetail(options)
    },
    /**
            ...
            data.forEach((flow, i) => {
                flow.forEach((node, j) => {
                    ...
                    if (params) {
                        ...
                        // “params.from 시점 이후부터 params.to 시점까지의 구간 전체가
                        // node의 페이지 열림 구간(pageStartTm ~ pageEndTm) 안에 완전히 포함되어 있을 때”
                            // 를 mxPageId가 같으면 찍도록 바꿈
                        else if (params.mxPageId && node.mxPageId) {
                            if (params.mxPageId === node.mxPageId) {
                    ...
                    const tooltipMap = frontUserflow.makeTooltipInfo(node)
    ...
    makeTooltipInfo(node) {
        ...
        const text = {
            ...
            serverType: trl('common.text.serverType'),
            reqUrl: trl('common.text.requestedUrl'),
            startTime: trl('common.text.pageStartTm'),
            endTime: trl('common.text.pageEndTm'),
            loadingTime: trl('common.text.pageLoadTm'),
            eventCount: trl('common.text.eventCount'),
            eventLoadTime: trl('common.text.eventLoadTm'),
            errorCount: trl('common.text.errorCount'),
            requestCount: trl('common.text.requestCount'),
            loginYn: trl('common.text.loginYn'),
            pageFlow: trl('common.text.pageflow'),
            pageType: trl('common.text.pagetype')
        }

        const serverTypeNm = frontUserflow.isEmpty(getServerNm(serverType)) ? '-' : i18next.tns('common.' + getServerNm(serverType))

        headerType = '<div class="sub_title">' + text.pageFlow + '</div>'
        deviceType = '<div class="content_title">' + (frontUserflow.isEmpty(reqUrl) ? '-' : getPageList(packageNm, serverType, reqUrl)) + '</div>'
        ...
        tooltipText += '<div><span class="maxy_tooltip_content_title">' + text.pageType + ': ' + '</span> <b>' +
            (frontUserflow.isEmpty(logType) ? '-' : util.logTypeToPageType(logType)[1]) + '</b></div>'
        tooltipText += '<div><span class="maxy_tooltip_content_title">' + text.startTime + ': ' + '</span> <b>' + (frontUserflow.isEmpty(pageStartTm) ? '-' : util.timestampToDateTime(pageStartTm)) + '</b></div>';
        tooltipText += '<div><span class="maxy_tooltip_content_title">' + text.endTime + ': ' + '</span> <b>' + (frontUserflow.isEmpty(pageEndTm) ? '-' : util.timestampToDateTime(pageEndTm)) + '</b></div>';
        tooltipText += '<div><span class="maxy_tooltip_content_title">' + text.loadingTime + ': ' + '</span> <b>' + (frontUserflow.isEmpty(intervaltime) ? '-' : util.convertTime(intervaltime, true, false, true)) + '</b></div>';
        tooltipText += '<div><span class="maxy_tooltip_content_title">' + text.eventCount + ': ' + '</span> <b>' + (frontUserflow.isEmpty(eventCount) ? '-' : util.comma(eventCount)) + '</b></div>';
        tooltipText += '</div>'

        // 오른쪽 데이터
        tooltipText += '<div class="maxy_tooltip_content">'
        tooltipText += '<div><span class="maxy_tooltip_content_title">' + text.eventLoadTime + ': ' + '</span> <b>' + (frontUserflow.isEmpty(eventIntervaltime) ? '-' : util.convertTime(eventIntervaltime, false)) + '</b></div>';
        tooltipText += '<div><span class="maxy_tooltip_content_title">' + text.requestCount + ': ' + '</span> <b>' + (frontUserflow.isEmpty(requestCount) ? '-' : util.comma(requestCount)) + '</b></div>';
        tooltipText += '<div><span class="maxy_tooltip_content_title">' + text.loginYn + ': ' + '</span><b>' +
            (frontUserflow.isEmpty(loginYn) ? '-' : loginYn) +
            '</b></div>'
        tooltipText += '<div><span class="maxy_tooltip_content_title">' + text.errorCount + ': ' + '</span><b>' +
            (frontUserflow.isEmpty(errorCount) ? '-' : util.comma(errorCount)) +
            '</b></div>'
        tooltipText += '</div>'
        tooltipText += '</div>'
        ...
        Handlebars.registerHelper('deviceInfo', function (val) {
            ...
            switch (val) {
                ...
                case 'simOperatorNm':
                    ...
                    break
                case 'osVer':
                    const osVer = '<span>' + result + '</span>'
                    return new Handlebars.SafeString(osVer)
                default:
                    break
            }
