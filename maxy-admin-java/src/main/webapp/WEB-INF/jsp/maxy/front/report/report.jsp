...
<div class="maxy_popup_common_wrap" id="maxyPopupWrap"></div><%--TODO 세션리플레이 테스트--%>
        ...
        func: {
            cmm: {
                ...
                setDownloadUrl(param) {
                    let url = "<c:url value="/fr/0000/downloadReportDataByRDB.maxy?"/>"
                    let errorCrashurl = "<c:url value="/fr/0000/downloadErrorCrashReportData.maxy?"/>"
            ...
            fetch: {
                ...
                scheduledEmailList() {
                    ...
                    ajaxCall('/fr/0000/getScheduledEmailList.maxy', {}, {
                        disableCursor: true
                    }).then((data) => {
                        // 테이블에 데이터 렌더링
                ...
                reportType() {
                    ...
                    return new Promise((resolve, reject) => {
                        // 보고서 조회 유형 가져오기
                        ajaxCall('/fr/0000/getReportType.maxy', {}, {
                            disableCursor: true
                        }).then((data) => {
                ...
                report() {
                    ...
                    document.cookie = "frontReportType=" + encodeURIComponent(reportType) + "; path=/; max-age=" + (60 * 60 * 24 * 30);

                    let url = "<c:url value="/fr/0000/maxyReport.maxy?"/>"
                    let searchUrl = new URLSearchParams(url.search)
            ...
            addEvent: {
                ...
                addPopup() {
                    ...
                    $('#btnSendEmail').on('click', function () {
                        ...
                        $('#emailSubject').val('MAXY Front Report (' + dateRange + ')')

                        // 기본 보고서 제목 설정
                        const nowDate = util.nowDate()
                        const nowTime = util.nowTime().replaceAll(':', '')
                        $('#reportSubject').val('MAXY_Front_Report_' + nowDate + '_' + nowTime)
                    ...
                    $('#btnScheduledEmail').on('click', function () {
                        ...
                        // 기본 제목 설정
                        $('#scheduledEmailSubject').val('MAXY Front Report')
                        // 기본 보고서 제목 설정
                        $('#scheduledReportSubject').val('MAXY_Front_Report')
                    ...
                    $('#btnSendEmailSubmit').on('click', function () {
                        ...
                        // 보고서 첨부해서 이메일 전송
                        ajaxCall('/fr/0000/sendReportByEmail.maxy', requestData, {disableCursor: true}).then(response => {
                            if (response.success) {
                    ...
                    $('#btnScheduledEmailSubmit').on('click', function () {
                        ...
                        // 예약 발송 등록 API 호출
                        ajaxCall('/fr/0000/upsertScheduledEmail.maxy', param).then(() => {
                            // 성공 메시지 표시
                    ...
                    $('#btnScheduledEmailDelete').on('click', function () {
                        ...
                        modal.show({
                            ...
                            confirm: true,
                            fn: () => {
                                ajaxCall('/fr/0000/deleteScheduledEmail.maxy', param, {
                                    disableCursor: true
                                }).then((data) => {
                                    // 테이블에 데이터 렌더링
                                    RT0000.func.draw.scheduledEmailList(data.emailList);
