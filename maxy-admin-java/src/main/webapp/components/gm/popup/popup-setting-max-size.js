class PopupSettingMaxSize {
    ...
    async init() {
        ...
        const source = await fetch(
            '/components/gm/popup/popup-setting-max-size.html')
    ...
    validate() {
        ...
        // PV Equalizer의 경우 1~30 사이의 값만 입력 가능
        if (id === 'pvequalizer' || id === 'pageview' || id === 'favorites') {
            if (util.isEmpty(maxSize) || maxSize <= 0 || maxSize > 30) {
