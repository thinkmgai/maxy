class MaxyFrontPopupAjaxResponse {
    constructor(options) {
        this.id = options.id
    ...
    initChart() {
        ...
        this.allChart = Highcharts.chart(v.id + '_chart', {
            chart: {
                type: 'column',
            },
            legend: {
    ...
    getListData(isClick) {
        ...
        // 대시보드 - 스캐터 차트인 경우엔 yFrom, yTo 존재함
        ...
        ajaxCall('/mf/0000/dashboard/network/list.maxy', param).then(res => {
            // 테이블에 데이터 설정
            const {data, chartData} = res
            this.table.setData(data)

            this.allChart.series[0].setData(chartData.chart)
