class MaxyFrontPopupUserInfo {
    ...
    async init() {
        ...
        const source = await fetch('/components/front/ua/popup/popup-user-info.html').then(res => res.text())
    ...
    getUserDetail() {
        ...
        ajaxCall('/fu/0000/user.maxy', param, {disableCursor: true})
            .then((data) => {
                cursor.hide()
                this.setData(data)
            })
    ...
    setData(data) {
        ...
        try {
            $target.find('#deviceModel').text(deviceModel + (webviewVer ?? '') || '-')
        } catch (e) {
