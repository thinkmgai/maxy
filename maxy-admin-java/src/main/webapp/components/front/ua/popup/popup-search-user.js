class MaxyFrontPopupSearchUser {
    constructor(options) {
        // 옵션에서 속성 초기화
    ...
    handleSearch() {
        ...
        ajaxCall('/fu/0000/users.maxy', searchParams)
            .then(data => {
                const users = data.users
                if (users.length === 1) {
                    // 단일 결과 - 즉시 검색 실행
                    this.executeMultipleSearch(users[0])
                    this.closePopup()
                } else {
                    // 복수 결과 - 테이블에 표시
                    this.data = users
    ...
    executeMultipleSearch(userData) {
        ...
        userAnalysis.func.getFlowData(multipleParams)
    }
    ...
    drawTable() {
        ...
        const columnNames = {
            'createdDate': trl('common.tableColumn.createdDate'),
            'updatedDate': trl('common.tableColumn.updatedDate'),
            'customerNo': trl('common.tableColumn.customerNo'),
            'deviceNo': trl('common.tableColumn.deviceNo'),
            'appVersion': trl('common.tableColumn.appVersion'),
            'operatingSystem': trl('common.tableColumn.operatingSystem'),
            'modelName': trl('common.tableColumn.modelName'),
            'userId': trl('common.tableColumn.userId')
        }
    ...
    closePopup() {
        // 현재 검색값 저장
        userAnalysis.func.setSearchKeyValue(
